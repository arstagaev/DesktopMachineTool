...
class PacketListener2 : SerialPortPacketListener {
...
private var incr = 0
private var incrX2 = 0
private var lastGauge : DataChunkG? = null


suspend fun coreParse_SecondEight(updData: ByteArray) = withContext(Dispatchers.IO) {
    ...
    when {
        ...
        updData.all { it == 0xFF.toByte() } -> {
            ...
            logGarbage("End Experiment! all it == 0xFF ${isExperimentStarts.value} contOfPacks: ${incrX2}")
            //incrX2 = 0
        }
        ...
        updData[7] in 16..31 -> {
            ...
            dataChunkCurrents2.emit(dchCurr)
            dataChunkGauges2.emit(lastGauge!!)
